...
enum MeetingSummaryClient {
    ...
    static func generateTitle(
        ...
        if backend == MeetingSummaryBackendOption.ollama.backend {
            return await generateTitleWithOllama(userPrompt: userPrompt, config: config)
        }

        if backend == MeetingSummaryBackendOption.lmStudio.backend {
            return await generateTitleWithLMStudio(
                userPrompt: userPrompt,
                config: config,
                titleInstructions: titleInstructions
            )
        ...
        if backend == MeetingSummaryBackendOption.customLLM.backend {
            return await generateTitleWithCustomLLM(
                userPrompt: userPrompt,
                config: config,
                titleInstructions: titleInstructions
            )
    ...
    private static func generateTitleWithOllama(userPrompt: String, config: AppConfig) async -> String? {
        let baseURLString = config.ollamaURL.trimmingCharacters(in: .whitespacesAndNewlines)
        let baseURL: URL
        if baseURLString.isEmpty {
            baseURL = defaultOllamaBaseURL
