...
final class GoogleCalendarClient {
    ...
    func parseEvent(_ item: [String: Any], calendarID: String) -> UnifiedCalendarEvent? {
        ...
        let calendarSummary = cachedCalendarList.first(where: { $0.id == calendarID })
        ...
        return UnifiedCalendarEvent(
            ...
            calendarName: calendarSummary?.summary ?? "Google Calendar",
            calendarSourceTitle: "Google Calendar",
            calendarColorHex: calendarSummary?.colorHex,
            attendees: attendees
        )
    }
