...
final class MuesliController: NSObject {
    ...
    func startMeetingRecording(
        ...
        updateMeetingNotificationVisibility()
        statusBarController?.setStatus("Starting meeting: \(resolvedStart.title)")
        statusBarController?.refresh()
    ...
    func startRecordingForExistingMeeting(id: Int64) {
        ...
        let meetingEnd = meeting.durationSeconds > 0
            ? meetingStart.addingTimeInterval(meeting.durationSeconds)
            : nil
        ...
        Task { @MainActor [weak self] in
            ...
            do {
                ...
                if self.meetingStartMeetingID == id {
                    self.resolveExistingMeetingAfterStartFailure(id: id, previousStatus: previousStatus)
                    self.cancelMeetingRecordingHotkeyToggleAfterFailedStart(meetingID: id)
                    self.meetingMonitor.resumeAfterCooldown()
                    self.meetingMonitor.refreshState()
                    self.statusBarController?.setStatus("Idle")
                    self.statusBarController?.refresh()
            ...
            self.finishMeetingStartAttempt(meetingID: id)
        }
    }
